...
<div class="ml-sm-5 mr-sm-5 ">
  ...
  <div class="row">
      ...
      <?php
      $best_membre = $pdo->query("SELECT sum(prix) AS somme, s.id_salle, m.id_membre, nom, prenom FROM salle s, commande c, produit p, membre m WHERE s.id_salle=p.id_salle AND p.id_produit=c.id_produit AND m.id_membre=c.id_membre group by m.id_membre order by somme DESC LIMIT 5");
      ?>
  ...
  <div class="row">
      ...
      <?php
      $best_membres = $pdo->query("SELECT count(id_commande) AS nb,titre, s.id_salle, id_commande, m.id_membre, nom, prenom FROM salle s, commande c, produit p, membre m WHERE s.id_salle=p.id_salle AND p.id_produit=c.id_produit AND m.id_membre=c.id_membre group by m.id_membre  order by nb DESC LIMIT 5");
...
$best_salles = $pdo->query("SELECT ROUND(AVG(note),1) AS moy, titre, s.id_salle, categorie from avis a LEFT JOIN salle s ON a.id_salle=s.id_salle GROUP BY titre ORDER BY moy DESC LIMIT 5");
?>  
    ...
    <div class="row">
        ...
        <?php
        $best_salle = $pdo->query("SELECT count(id_commande) AS nb,titre, s.id_salle, id_commande, categorie FROM salle s RIGHT JOIN produit p ON s.id_salle=p.id_salle RIGHT JOIN commande c ON p.id_produit=c.id_produit GROUP BY titre ORDER BY nb DESC LIMIT 5");
